...
def backtest(df: pd.DataFrame, 
    ...
    for i in range(len(highs)):
        ...
        for side in ["resistances", "supports"]:
            ...
            # check whether the price groups are still vaid or not
            ...
            for sup_res in resistances_supports[side]:
                entry_condition = row["close"][i] > sup_res["price"] if side == "resistance" else row["close"][i] < sup_res["price"]
            ...
            #  check pnl
            if trade_side == 1:
                if row["close"][i] >= entry_price * (1 + take_profit / 100) \
                or row["close"][i] <= entry_price * (1 - stop_loss / 100):
